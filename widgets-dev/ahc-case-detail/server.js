(function() {
  ...
  // Who may manage the watch list. Note: SP widget server scripts run in global
  // scope, where GlideRecord writes are NOT ACL-checked — so a field-level ACL
  // test (as we tried first) both blocked the UI for legitimate owners AND
  // wouldn't have gated the actual write anyway. Gate on involvement instead:
  // the case contact/opener, or a customer_admin over the account, or anyone
  // the platform says can write the record (agents/admins).
  function canManageWatchers(rec) {
    var meId = gs.getUserID();
    return rec.getValue('opened_by') === meId
        || rec.getValue('contact')   === meId
        || gs.hasRole('sn_customerservice.customer_admin')
        || gs.hasRole('admin')
        || rec.canWrite();
  }
  ...
  if (input && input.action) {
    ...
    if (actGr.get(sysId) && actGr.canWrite()) {
      ...
      if (input.action === 'add_comment' && input.comment) {
        ...
        data.actionDone = !!actGr.update();
      } else if (input.action === 'add_watcher' && canManageWatchers(actGr)) {
        // Append to the case watch list (deduped). Accepts either a real user
        // sys_id (from the colleague picker) or a raw email address — watch_list
        // is a glide_list that stores both.
        var token = '';
        if (input.watcherId && /^[0-9a-f]{32}$/i.test(String(input.watcherId))) {
          token = String(input.watcherId);
        } else if (input.watcherEmail && EMAIL_RE.test(String(input.watcherEmail).trim())) {
          token = String(input.watcherEmail).trim();
        }
        if (token) {
          var addList = (actGr.getValue('watch_list') || '').split(',')
            .map(function(s) { return s.trim(); }).filter(function(x) { return x; });
          var dup = addList.some(function(x) { return x.toLowerCase() === token.toLowerCase(); });
          if (!dup) {
            addList.push(token);
            actGr.watch_list = addList.join(',');
            data.actionDone = !!actGr.update();
          } else {
            data.actionDone = true; // already watching — treat as success
          }
        } else {
          data.actionError = true; // neither a valid sys_id nor a valid email
        }
      } else if (input.action === 'remove_watcher' && input.watcherId && canManageWatchers(actGr)) {
        // watcherId is the stored token (sys_id or email); match case-insensitively
        var tok = String(input.watcherId).toLowerCase();
        var remList = (actGr.getValue('watch_list') || '').split(',')
          .map(function(s) { return s.trim(); })
          .filter(function(x) { return x && x.toLowerCase() !== tok; });
        actGr.watch_list = remList.join(',');
        data.actionDone = !!actGr.update();
      }
    } else {
  ...
  // ── Watchers (watch_list) ────────────────────────────────────────────────
  // Lets a submitter add a colleague or email so case updates reach them (e.g.
  // before going on leave). canWatch gates the UI on involvement with the case.
  data.canWatch = canManageWatchers(gr);

  var watchIds     = (gr.getValue('watch_list') || '').split(',').filter(function(x) { return x; });
  var watchUserIds = [];
  var watchEmails  = [];
  watchIds.forEach(function(tok) {
    if (/^[0-9a-f]{32}$/i.test(tok)) watchUserIds.push(tok);
    else watchEmails.push(tok); // watch_list may also hold raw email strings
  });

  var watchers = [];
  if (watchUserIds.length) {
    var wGr = new GlideRecord('sys_user');
    wGr.addQuery('sys_id', 'IN', watchUserIds.join(','));
    wGr.query();
    while (wGr.next()) {
      watchers.push({
        sys_id: wGr.getUniqueValue(),
        name:   wGr.getValue('name') || wGr.getValue('user_name'),
        email:  wGr.getValue('email') || ''
      });
    }
  }
  watchEmails.forEach(function(em) { watchers.push({ sys_id: em, name: em, email: em }); });
  data.watchers = watchers;

  // Quick "Watch this" toggle — shown on cases the current user did NOT open.
  // Self-watch reuses add_watcher/remove_watcher with the user's own sys_id.
  var meId = gs.getUserID();
  data.currentUserId = meId;
  data.isOpener      = (gr.getValue('opened_by') === meId);
  data.isWatching    = watchUserIds.indexOf(meId) !== -1;

  // Candidate list for the "Add watcher" picker: active users on this case's
  // account, minus whoever is already watching. Scoping to the account keeps an
  // external customer to adding only their own colleagues.
  var watcherOptions = [];
  var accountSysId = gr.getValue('account');
  if (data.canWatch && accountSysId) {
    var already = {};
    watchUserIds.forEach(function(id) { already[id] = true; });
    var oGr = new GlideRecord('sys_user');
    oGr.addQuery('company', accountSysId);
    oGr.addActiveQuery();
    oGr.orderBy('name');
    oGr.setLimit(200);
    oGr.query();
    while (oGr.next()) {
      var oid = oGr.getUniqueValue();
      if (already[oid]) continue;
      watcherOptions.push({
        sys_id: oid,
        name:   oGr.getValue('name') || oGr.getValue('user_name'),
        email:  oGr.getValue('email') || ''
      });
    }
  }
