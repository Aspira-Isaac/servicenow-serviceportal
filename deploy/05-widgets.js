...
function read(widgetDir, filename) {
...
module.exports = async function deployWidgets(ctx) {
  for (const w of WIDGETS) {
    const { sys_id } = await upsert('sp_widget', 'id', w.id, {
      name: w.name,
      id: w.id,
      template: read(w.dir, 'template.html'),
      client_script: read(w.dir, 'client.js'),
      script: read(w.dir, 'server.js'),
      css: read(w.dir, 'style.scss'),
      public: true
    });
    ctx[w.ctxKey] = sys_id;
